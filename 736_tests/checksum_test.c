...
int main ()
{
    ...
    while (1) {
...
#if FLETCHER
        checksum1 = fletcher32((uint16_t *) block1, 4096);
#endif
#if CRC
        checksum1 = crc32(~0, (uint16_t *) block1, 4096);
#endif
        ...
        clock_gettime(CLOCK_MONOTONIC, &start);
#if FLETCHER
        checksum2 = fletcher32((uint16_t *) block2, 4096);
#endif
#if CRC
        checksum2 = crc32(~0, (uint16_t *) block2, 4096);
#endif
        clock_gettime(CLOCK_MONOTONIC, &end);
        fletcher32_time += ((end.tv_nsec - start.tv_nsec) + ((end.tv_sec - start.tv_sec) * 1000000000));
